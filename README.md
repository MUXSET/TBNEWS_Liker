...
> **🚀 v2.2.0 新体验升级！**
> 新增启动时无感核算当月文章总数功能，以及“强制直连”选项一键绕过各类企业终端网络审计环境，实现更顺畅的挂机体验！
...
### v2.2.0 (2026-03-11) — 实用功能升级
- 🏷️ **名称更新**：项目正式更名为「TBE+ 阅赞助手」。
- 🌐 **网络互通性优化**：新增「强制直连绕过代理」选项，解决企业终端内网审计环境引发的 `WinError 10013` 连接重置及 SSL 拦截报错，实现复杂内网顺畅挂机。
- ⏱️ **自动化体验再升级**：新增「启动时自动统计本月文章总数」，开启后每次开盖启动会在后台静默核算当月大盘进度更新UI卡片，无需再手动点击扫描。
- 🐛 **修复跳转拦截**：修复 SSO 登录跳转时因为原始标签页的 `target="_blank"` 导致被拦截阻断的问题。
